"""MEWCODE.md 三层加载 + @include 展开（ch09 F1-F8）."""
...
class InstructionResult:
    """三层 MEWCODE.md 加载 + @include 展开的最终结果。"""
...
def load_instructions(
    ...
    """启动时加载三层 MEWCODE.md 并展开 @include。
    ...
    user_mewcode_dir = Path.home() / ".mewcode"

    # 三层路径（F1）：优先级从高到低
    layers: list[tuple[Path, Path]] = [
        (project_root / "MEWCODE.md", project_root),                    # ① 项目级
        (project_root / ".mewcode" / "MEWCODE.md", project_root),       # ② 项目配置级
        (user_mewcode_dir / "MEWCODE.md", user_mewcode_dir),            # ③ 用户级
    ]
...
def _load_layer(path: Path, read_file: Callable[[Path], str | None]) -> str | None:
    """读单层 MEWCODE.md，不存在或编码错误返回 None。"""
    try:
        if not path.is_file():
            return None
    except OSError:
...
def _expand_includes(
    ...
    Args:
        ...
        depth: 当前嵌套深度（MEWCODE.md 本身为第 1 层）。
        read_file: 文件读取函数。
