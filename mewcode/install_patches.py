...
def _patch_win32_vk0() -> None:
    ...
    if path is None:
        logger.warning("[mewcode] textual.drivers.win32 not found, skip VK=0 patch")
        return

    content = path.read_text(encoding="utf-8")

    if _PATCH_VK0_MARKER in content:
        logger.debug("[mewcode] patch win32_vk0 already applied")
        return
    ...
    if original not in content:
        logger.warning(
            "[mewcode] win32_vk0: upstream win32.py changed — "
    ...
    logger.info(
        "[mewcode] patch win32_vk0 applied — CJK IME Shift+symbol events "
...
def _patch_cells_eaw() -> None:
    ...
    if path is None:
        logger.warning("[mewcode] rich.cells not found, skip EAW patch")
        return

    content = path.read_text(encoding="utf-8")

    if _PATCH_EAW_MARKER in content:
        logger.debug("[mewcode] patch cells_eaw already applied")
        return
    ...
    if old_import in content:
        ...
        logger.warning(
            "[mewcode] cells_eaw: cannot find import block in cells.py"
        )
    ...
    if old_fallback1 in content:
        ...
        logger.warning(
            "[mewcode] cells_eaw: cannot find first return-1 fallback in cells.py"
        )
    ...
    if old_fallback2 in content:
        ...
        logger.warning(
            "[mewcode] cells_eaw: cannot find second return-1 fallback in cells.py"
        )
    ...
    if changed:
        ...
        path.write_text(content, encoding="utf-8")
        logger.info(
            "[mewcode] patch cells_eaw applied — EAW='A' characters "
            "now return width=2 on CJK terminals"
        )
    else:
        logger.warning(
            "[mewcode] cells_eaw: no modifications were made — "
...
def _backup(path: Path) -> None:
    """创建 .mewcode.bak 备份（如果尚未备份）。"""
    bak = path.with_suffix(path.suffix + ".mewcode.bak")
    if not bak.exists():
