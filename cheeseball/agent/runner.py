...
from cheeseball.agent.collector import StreamingCollector
...
from cheeseball.agent.executor import ToolExecutor
from cheeseball.agent.state_machine import AgentState, LoopStateMachine
from cheeseball.core.conversation import Conversation
from cheeseball.permission import (
    PermissionEngine,
    Verdict,
    categorize,
    Mode,
)
from cheeseball.prompt import SystemPromptBuilder
from cheeseball.provider import BaseProvider
from cheeseball.provider import PromptTooLongError
...
class Agent:
    ...
    async def run(
        ...
        if user_input.startswith(_PRINT_PREFIX):
            ...
            # 写入文件
            out_dir = Path("D:/work/cheeseball/testChat")
...
def _cycle_mode_fallback(mode: Mode, direction: int) -> Mode:
    """引擎未初始化时手动循环模式。"""
    from cheeseball.permission import cycle_mode
