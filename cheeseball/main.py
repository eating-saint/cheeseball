"""Cheeseball 程序入口：参数解析 → 加载配置 → 启动 TUI（ch03 装配工具系统，ch08 上下文管理，ch09 记忆与持久化）."""
...
from cheeseball.config.loader import ConfigError, load_config
from cheeseball.context.config import resolve_context_window
from cheeseball.context.manager import ContextManager
from cheeseball.core.conversation import Conversation
from cheeseball.instruction.loader import load_instructions
from cheeseball.memory.manager import MemoryManager
from cheeseball.provider import create_provider
from cheeseball.session import new_session_id
from cheeseball.session.archiver import SessionArchiver
from cheeseball.session.scanner import cleanup_expired
from cheeseball.tool import new_default_registry
...
def main() -> None:
    ...
    # 6. 创建工具注册中心（工作目录 = 启动 cheeseball 时的当前目录）
    cwd = os.getcwd()
    registry = new_default_registry(cwd)

    # 7. 加载 MCP 配置
    from cheeseball.mcp.config import load_mcp_servers
    ...
    memory_mgr = MemoryManager(
        project_memory_dir=Path.cwd() / ".cheeseball" / "memory",
    ...
    session_dir = Path.cwd() / ".cheeseball" / "sessions" / session_id
    ...
    from cheeseball.install_patches import ensure_patches

    ensure_patches()

    # 15¾. [ch10] 初始化命令注册中心
    from cheeseball.cmd import CommandRegistry, CommandDef, CommandKind
    ...
    from cheeseball.ui.app import CheeseballApp
